              ...
              <div class="row">
                ...
                <div class="col-sm-6">
                  <a href="{{route('AddBanner')}}">
                    <button type="button" class="btn btn-secondary m-1 pull-right">Add Banner</button>
                  </a>
                </div>
              </div>
               <hr>
                ...
                <table class="table">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td class="FontSize"><img style='height:80px; width:200px;' src="{{ url('images/banner/'.$row->image) }}"></td>
                      <td class="FontSize">{{ date('d-m-Y',strtotime($row->created_at)) }}</td>
                      <td class="FontSize">@if($row->status==1) {{"active"}} @else {{ "inactive"}} @endif</td>
                      <td class="FontSize">
                          <a href="{{route('EditBanner',['id'=>base64_encode($row->id)])}}">
                            <button class="btn btn-success"><i class="fa fa-edit"></i></button>
                          </a>
                          <a href="{{route('DeleteBanner',['id'=>base64_encode($row->id)])}}" onclick="return confirm('Are you sure?')">
