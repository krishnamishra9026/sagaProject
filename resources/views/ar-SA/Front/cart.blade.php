            ...
            <div class="col-lg-12 table-responsive mb-5">
                <table class="table table-light table-borderless table-hover text-center mb-0">
                    <thead class="thead-dark">
                        <tr>
                            <th>@if(session('locale')=='en')  Remove @else إإزالة  @endif</th>
                            <th>@if(session('locale')=='en')  Total @else المجموع@endif</th>
                            <th>@if(session('locale')=='en')  ADD @else  اضافة @endif</th>
                            <th>@if(session('locale')=='en')  PRICE @else السعر    @endif</th>
                            <th class="text-right">@if(session('locale')=='en')  PRODUCT NAME @else  اسم المنتج   @endif</th>
                            <th class="text-right" style="width: 170px;">@if(session('locale')=='en')  IMAGE @elseصورة   @endif</th>
                    ...
                    <tbody class="align-middle text-center">
                        ...
                        <tr>
                            ...
                            <td class="align-middle">
                                <h5>{{ $item->price*$item->quantity }} ريال سعودي</h5>
                            </td>
                            <form class="form" action="{{ route('cart.update') }}" method="POST">
                                ...
                                <td class="qty qua-col first-row" data-title="Qty" style="width: 120px; padding-top: 45px;">
                                  <div class="input-group">
                                      ...
                                      <div class="button plus"><button class="btn btn-sm btn-primary btn-plus" id="quantity_plus" onclick="increaseQty('quantity', '<?php echo $i ?>',{{ $item->id}},{{ $item->quantity }});" ><i class="fa fa-plus"></i></button></div>
                                  </div>
                                </td>
                            </form>
                            ...
                            <td class="align-middle">
                                <div class="d-flex align-items-center justify-content-center mt-2">
                                    <h6 class="text-muted ml-2"><del>{{ $item->attributes->OldPrice }} ريال سعودي </del></h6>&nbsp;&nbsp;<h5>{{ $item->price }} ريال سعودي</h5>
                                </div>
                            </td>
                            <td class="align-middle text-right">{{ isset($product->name) ? $product->name : '' }}</td>
                            <td style="width: 170px;" class="align-middle text-right">
                ...
                <div class="text-right">
                    ...
                    @if(!session('user_id'))
                    <a href="{{route('Login')}}" class="btn btn-primary font-weight-bold my-3 py-3">@if(session('locale')=='en')  Proceed To Checkout @else  اكمل عملية الشراء  @endif</a>
                    @else
                    <a href="{{route('AuthCheck')}}" class="btn btn-primary font-weight-bold my-3 py-3">@if(session('locale')=='en')  Proceed To Checkout @else اكمل عملية الشراء @endif</a>
                    @endif
                </div>
            </div>
        </div>
